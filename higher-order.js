...
//Code Here
let totalPopulation = populations.reduce((prev, now) => prev + now);
...
let orderTotals = orders.map((obj) => (1 + obj.tax) * obj.price);
...
let bobsTotal = purchases.filter((obj) => obj.owner === "Bob").reduce((prev, pres) => prev + pres.price, 0);
